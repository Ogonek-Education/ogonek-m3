<script lang="ts" generics="T">
  ...
  const toggleDropdown = (event: MouseEvent) => {
    ...
    if (
      ...
      multiSelectContainer.contains(event.target as Node)
    ) {
      show = !show;
    } else {
  ...
  function handleToggleActiveItem() {
    ...
    if (!show) {
      ...
      if (activeItem !== null)
        selectOption(activeItem, new MouseEvent("click")); // Pass a dummy MouseEvent
    }
  }
  ...
  const {
    ...
    svg,
  } = multiSelect({ disabled, grouped: !!group });
</script>
...
<div
  ...
  tabindex="0"
  role="listbox"
  class={base({ size, class: clsx(className) })}
>
  {#if !selectItems.length}
    <span class={placeholderSpan({ class: clsx(classes?.placeholder) })}
      >{placeholder}</span
    >
  {/if}
  <span class={select({ class: clsx(classes?.span) })}>
    {#if selectItems.length}
  ...
  <div class="ms-auto flex items-center gap-2">
    {#if selectItems.length}
      <CloseButton
        {size}
        color="none"
        class={close({ class: clsx(classes?.close) })}
        {disabled}
      />
    {/if}

    <svg
      class={clsx(svg(), disabled && "cursor-not-allowed", classes?.svg)}
      aria-hidden="true"
  ...
  {#if show}
    <div
      role="presentation"
      class={dropdown({ class: clsx(styling.dropdown) })}
    >
