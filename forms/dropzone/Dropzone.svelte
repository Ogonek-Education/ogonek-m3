<script lang="ts">
  ...
  const handleDrop: DragEventHandler<HTMLLabelElement> = function (
    this: Window,
    event,
  ) {
  ...
  const handleDragOver: DragEventHandler<HTMLLabelElement> = function (
    this: Window,
    event,
  ) {
    event.preventDefault();
...
<label
  class={dropzone({ class: clsx(className) })}
  ondrop={handleDrop}
