...
export const slider = tv({
	slots: {
		root: 'slider-root relative min-w-40 [block-size:var(--handle-height)] [print-color-adjust:exact] [--m3-slider-track-out-shape:0.5rem] [--m3-slider-track-in-shape:0.125rem] [--m3-slider-handle-shape:var(--m3-shape-full)] [--functional-width:calc(100%-2*(0.25rem+0.125rem))] [--handle-left:calc(50%+var(--functional-width)*var(--handle)-0.125rem-0.375rem)] [--handle-center:calc(50%+var(--functional-width)*var(--handle))] [--handle-right:calc(50%+var(--functional-width)*var(--handle)+0.125rem+0.375rem)]',
		input:
		...
		stopFill: 'bg-[var(--m3c-on-primary)] peer-disabled:bg-[var(--m3c-inverse-on-surface)]',
		stopRest:
			'bg-[var(--m3c-primary)] peer-disabled:bg-[--translucent(var(--m3c-on-surface),0.38)]',
		handle:
			'absolute inset-y-0 left-[var(--handle-center)] w-1 rounded-[1.25rem] bg-[var(--m3c-primary)] pointer-events-none peer-focus-visible:outline peer-focus-visible:outline-[4px] peer-focus-visible:outline-[var(--m3c-on-secondary-container)] peer-focus-visible:outline-offset-[5px] peer-focus-visible:z-[2] peer-focus-visible:w-0.5 peer-active:w-0.5 peer-disabled:bg-[--translucent(var(--m3c-on-surface),0.38)] [@media(forced-colors:active)]:bg-[selecteditem] peer-disabled:[@media(forced-colors:active)]:bg-[graytext]',
		value:
			'absolute flex min-w-12 items-center justify-center rounded-[var(--m3-slider-handle-shape)] bg-[var(--m3c-inverse-surface)] px-4 py-3 text-[var(--m3c-inverse-on-surface)] md-sys-typescale-label-large [inset-inline-start:var(--handle-center)] [inset-block-end:calc(var(--handle-height)+4px)] opacity-0 pointer-events-none select-none z-[1] peer-hover:opacity-100 peer-focus-visible:opacity-100 peer-active:opacity-100 [@media(forced-colors:active)]:border-2 [@media(forced-colors:active)]:border-[selecteditem] [@media(forced-colors:active)]:overflow-hidden'
	},
	variants: {
		size: {
