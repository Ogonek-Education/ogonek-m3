<script lang="ts">
	...
	const { iconProps, label, href, badge, class: className, ...rest }: NavItemProps = $props();

	const active = $derived(
		page.url.pathname === href || (href !== '/' && page.url.pathname.startsWith(href))
	);
	const { base, content, icon, iconContainer, label: labelClass } = $derived(navitem({ active }));
</script>

<a {href} {...rest} class={clsx(base(), className)} data-cy={`rail-${label}`}>
	<div class={content()}>
		<div class={iconContainer()}>
			<Icon {...iconProps} class={icon()} wght={active ? 600 : 400} fill={active ? 1 : 0} />
			{#if badge}
